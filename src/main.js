...
import { mergeProducts, mergeWithUzumProducts, splitDifferences } from "./utils/helper.js";
import {
  getUzumProducts,
  getUzumStocks,
...
import db, { connect, getDB } from "./service/db.service.js";
...
app.get("/products/table", async (req, res) => {
  try {
    // let billzProducts = [];
    // try {
    //   const billzToken = await ensureBillzLoginIfStale();
    //   if (billzToken) {
    //     billzProducts = await getProducts({ token: billzToken });
    //   } else {
    //     console.warn(
    //       "Billz token unavailable; dropdown options will be empty."
    //     );
    //   }
    // } catch (err) {
    //   console.error("Failed to fetch Billz products:", err?.message || err);
    //   billzProducts = [];
    // }
    ...
    const buildLink = (pageNumber) => {
      ...
      return `/products/table${queryString ? `?${queryString}` : ""}`;
    };
    ...
    for (let p = paginationStart; p <= paginationEnd; p += 1) {
      if (p === page) {
        ...
        paginationLinks.push(
          `<a class="pager-link" href="${buildLink(p)}">${escapeHtml(p)}</a>`
        );
      }
    }
    ...
    if (page < totalPages) {
      paginationLinks.push(
        `<a class="pager-link" href="${buildLink(page + 1)}">Next &raquo;</a>`
      );
    }
    ...
    const rowsHtml = products
      .map((product) => {
        ...
        const amountDataset = Number.isFinite(amountRaw)
          ? String(amountRaw)
          : "";
        const priceDataset = Number.isFinite(priceRaw) ? String(priceRaw) : "";
        const updatedAtRaw =
...
<html lang="en">
  ...
  <body>
    ...
    <form class="toolbar" method="get" action="/products/table">
      <label>
        Search
...
app.post("/products/update", async (req, res) => {
  try {
    ...
    const price =
      priceRaw === "" || priceRaw == null ? null : Number.parseFloat(priceRaw);
...
async function main() {
  ...
  billzProducts = [...products];
  const collectComparison = compareCollectWithBillz(collectData, products);
  ...
  app.get("/collectcom", (req, res) => {
    res.json(collectComparison);
  });

  console.log("Collect & Billz matches:", collectComparison.both.length);
  console.log("Collect only Uzum:", collectComparison.onlyUzum.length);
  ...
  const datas = await database.collection("products").find({
    barcode_billz: { $exists: true, $ne: null },
  }).toArray();
  const syncProduct = [...collectComparison.both, ...datas];
  const mergeProds = mergeWithUzumProducts(products, syncProduct);

  app.get("/prod", (req, res) => {
    res.json(syncProduct);
  });
  console.log(
    `Uzum-only products synced. Inserted: ${syncResult.inserted}, Updated: ${syncResult.updated}`
  );
